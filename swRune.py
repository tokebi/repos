...
class SwRune():
	...
	def getUriComment(self):
		uri = ""
		uricomment = ""
		if self.data["reaDo"] == 6: # レア
			if self.data["sec_eff"][0][0] in [1,3,5]: # 1:体、3:攻、5:防
				uri = "売"
				uricomment = "1番実数"
			...
			if self.data["slot_no"] in [2,4,6]:
				...
				uricomment = ""
			if uri == "":
				if self.data["sec_eff"][0][0] == 8 or self.data["sec_eff"][1][0] == 8:# 速度
					if self.data["class"] == 5 and \
						((self.data["sec_eff"][0][0] == 8 and self.data["sec_eff"][0][1] == 3) or \
						 (self.data["sec_eff"][1][0] == 8 and self.data["sec_eff"][1][1] == 3)):# 速度:3
						uri = "売"
						uricomment = "速度3"
					else:
						uri = ""
						uricomment = ""
				elif self.data["slot_no"] in [2,4,6]: # スロットが2,4,6
					uri = ""
					uricomment = ""
				elif self.data["sec_eff"][0][0] in [9, 10] and self.data["sec_eff"][1][0] in [9, 10]: # 9:クリ、10:ダメ
					uri = ""
					uricomment = ""
				else:
					uri = "売3"
					uricomment = "速度なし、クリなし、ダメなし"
		if self.data["reaDo"] == 9: # ヒーロー
